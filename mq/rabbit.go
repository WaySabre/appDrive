...
func ConnectScript() {
	...
	addr := "amqp://" + conf.MqUser + ":" + conf.MqPwd + "@" + conf.MqUrl + "/"
	AmqpServer = Service{
		AmqpUrl:       addr,
		ConnectionNum: 2,
		ChannelNum:    5,
...
func (S *Service) declareExchange(ch *amqp.Channel, exchangeName string, channelId int) (*amqp.Channel, amqp.Queue) {
	...
	// 	nil,          // arguments
	// )
	q, isErr := ch.QueueDeclare(exchangeName, true, false, false, true, nil)
	if isErr != nil {
		ch, q = S.reDeclareExchange(channelId, exchangeName, isErr)
	}
	return ch, q
}

func (S *Service) reDeclareExchange(channelId int, exchangeName string, err error) (ch *amqp.Channel, q amqp.Queue) {
	...
	if strings.Index(err.Error(), "channel/connection is not open") >= 0 {
		...
		// )
		var err error
		q, err = cha.ch.QueueDeclare(exchangeName, true, false, false, true, nil)
		if err != nil {
...
func (S *Service) publish(channelId int, ch *amqp.Channel, routeKey string, data string) (err error) {
	err = ch.Publish(
		"",       // exchange
	...
	if err != nil {
		if strings.Index(err.Error(), "channel/connection is not open") >= 0 {
			err = S.rePublish(channelId, err, routeKey, data)
		}
	}
	return
}

func (S *Service) rePublish(channelId int, errmsg error, routeKey string, data string) (err error) {
	//fmt.Println("rePublish")

	ch, q := S.reDeclareExchange(channelId, routeKey, errmsg)
	routeKey = q.Name
	err = ch.Publish(
		"",       // exchange
		routeKey, //severityFrom(os.Args), // routing key
...
func (S *Service) QueueDeclare(exchangeName string, notice interface{}) (message interface{}, puberr error) {
	...
	var q amqp.Queue
	ch, q = S.declareExchange(ch, exchangeName, channelId)
	data := S.dataForm(notice)
	var tryTime = 1
	for {
		puberr = S.publish(channelId, ch, q.Name, data)
		if puberr != nil {
...
func (S *Service) Consume(queueName string) (<-chan amqp.Delivery, error) {
	...
	ch, _ = S.declareExchange(ch, queueName, channelId)
	_ = ch.Qos(1, 0, true)
